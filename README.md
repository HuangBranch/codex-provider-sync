...
- 同步时保护含 `encrypted_content` 且来自其它 provider/account 的会话，不再强行改写 provider
- 自动备份
- 列出备份并显示备份日期
- 从备份恢复
- 删除备份，删除前会显示应用内确认弹窗
...
- 含 `encrypted_content` 的历史会话跨 provider/account 后，通常只能恢复列表可见性，继续对话或 compact 仍可能失败；v2.2.2 起这些跨 provider/account encrypted 会话默认不会被强行改写 provider。
